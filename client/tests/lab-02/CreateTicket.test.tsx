...
describe("Create Ticket", () => {
  ...
  it("uses the selected requester and displays the official ticket number after submission", async () => {
    ...
    expect(screen.getByRole("textbox", { name: "Requester" })).toHaveAttribute(
      "readonly",
    );
    expect(screen.getByRole("textbox", { name: "Ticket Number" })).toHaveValue(
      "Generated after submission",
    );
    expect(screen.getByRole("textbox", { name: "Ticket Date" })).toHaveValue(
      "Set when saved",
    );
    ...
    expect(
      await screen.findByText("Ticket TKT-2026-000042 has been created"),
    ).toBeInTheDocument();
  ...
  it("shows field validation and preserves entered data after a safe API failure", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Submit Ticket" }));
    expect(
      await screen.findByText("Summary must contain 5 to 120 characters."),
    ).toBeInTheDocument();
    await completeForm(user);
    await user.click(screen.getByRole("button", { name: "Submit Ticket" }));
    expect(await screen.findByRole("alert")).toHaveTextContent(
      /could not create your ticket/i,
    );
  ...
  it("uploads selected files after creation and keeps a safe retry path for a failed file", async () => {
    ...
    const failed = new File(["failed"], "failed.pdf", {
      type: "application/pdf",
    });
    await user.upload(screen.getByLabelText("Select files"), [
      evidence,
      failed,
    ]);
    await user.click(screen.getByRole("button", { name: "Submit Ticket" }));
    ...
    expect(
      screen.getByText(/1 attachment uploaded successfully/i),
    ).toBeInTheDocument();
    expect(screen.getByRole("alert")).toHaveTextContent(/failed\.pdf/i);
    expect(
